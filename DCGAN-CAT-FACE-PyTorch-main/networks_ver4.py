...
def define_D(input_nc, ndf, n_layers_D=3, norm='batch', use_sigmoid=False, gpu_ids=[], patch=False):
	netD = None
	use_gpu = len(gpu_ids) > 0
	norm_layer = get_norm_layer(norm_type=norm)
...
class Unet_resize_conv(nn.Module):
	...
	def depth_to_space(self, input, block_size):
		...
		output = torch.stack(stcak,0).transpose(0,1).permute(0,2,1,3,4).resize(batch_size, s_height, s_width, s_depth)
