...
def train_fn(dataloader_target, gen , disc , opt_gen , opt_disc , fixed_noise, D_loss,G_loss):
    ...
    for epoch in range(1, config.NUM_EPOCHS + 1):
        ...
        for step,(real,_) in enumerate(dataloader_target):
            ...
            input_noise = torch.ones(batch_size, 3, 64, 64, dtype=torch.float, requires_grad=True).cuda()
            ...
            gen_imag = gen(input_noise)
            fake_output = disc(gen_imag.detach())
            d_fake_loss = D_loss(fake_output, torch.zeros_like(fake_output))
            d_fake_loss.backward()
            d_loss = d_fake_loss + d_real_loss
            opt_disc.step()

            opt_gen.zero_grad()
            fake_output = disc(gen_imag)
            g_loss = G_loss(fake_output, torch.ones_like(fake_output))
            g_loss.backward()
            opt_gen.step()
...
def main():
    ...
    input_fixed_noise = torch.ones(1, 3, 400, 592, dtype=torch.float, requires_grad=False).cuda()
