...
def train_fn(dataloader_target, gen , disc , opt_gen , opt_disc , fixed_noise, D_loss,G_loss):
    ...
    for epoch in range(1, config.NUM_EPOCHS + 1):
        ...
        for step,(real,_) in enumerate(dataloader_target):
            ...
            input_noise = torch.ones(batch_size, 3, 400, 592, dtype=torch.float).cuda()
        ...
        if epoch % config.SAVE_IMG_FREQ == 0:
            with torch.no_grad():
                ...
                plot_examples( "fake_ep" + str(epoch) + ".png" , "saved_samples_target/" , img_grid_fake)
...
def main():
    ...
    dataloader_target = getDataLoader(config, transforms, config.ROOT_TARGET)
